...
def plot_decision_boundary(model, X, y):
    ...
    plt.contourf(xx, yy, Z, cmap=plt.colormaps.get_cmap("viridis"))
    plt.ylabel("x2")
    plt.xlabel("x1")
    plt.scatter(X[0, :], X[1, :], c=y, cmap=plt.colormaps.get_cmap("viridis"))
...
def generate_spiral_planar_dataset():
    ...
    X = np.zeros(
        (m, D)
    )  # data matrix where each row is a single example, 2 columns rep the coordindates
    Y = np.zeros((m, 1), dtype="uint8")  # labels vector (0 for red, 1 for blue)
    a = 4  # maximum ray of the flower
    ...
    for j in range(2):
        ...
        ]  # Stacks these coordinates into a 2D X array, to specific positions within the X array, concatenates arrays column-wise
        Y[ix] = j
